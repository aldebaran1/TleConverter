...
def tle2eci(l1, l2, year, month, day, hour, minute, secunde):
	...
	position, velocity = satellite.propagate(year, month, day, hour, minute, secunde)
...
def eci2ecef(x, y, z, year, month, day, hour, minute, secunde):
	"""
	...
	GST = radians(getGST(year, month, day, hour, minute, secunde))
...
def azimuthAndEvevation(s, e, z, ro):
	"""
	...
	El = degrees(asin(z / ro))
	Az = degrees(atan2(e, -s))
	#Azimuth angle is in range [0, 360] degrees
	Az = Az % 360
	#
	#print "AE position: ", Az, El, ro
	return Az, El, ro

def getazel(year, month, day, hour, minute, sec):
	"""
	...
	sat_eci = tle2eci(lines[0], lines[1], year, month, day, hour, minute, sec)
	satellite_location_ecef = eci2ecef(sat_eci[0], sat_eci[1], sat_eci[2], year, month, day, hour, minute, sec)	
	...
	final_pos = azimuthAndEvevation(local_coordinate_system_satellite_position[0], local_coordinate_system_satellite_position[1],
		local_coordinate_system_satellite_position[2], local_coordinate_system_satellite_position[3])
...
def sec_to_time(seconds):
    ...
    mins = sec / 60
    sec -= 60*mins
    print hrs, mins, sec, seconds
    return hrs, mins, sec 

if __name__ == '__main__':
	i = 0
	while i < 86400:
		(hour, minute, secunde) = sec_to_time(i)
		#print y, mn, d, h, m, s 
		
		t = ephem.now()
		(year, month, day, h, m, s) = t.tuple()
		#secunde += i
		#print y, mn, d, h, min, sec
		#print year, month, day, hour, minute, secunde
		aer = getazel(year, month, day, hour, minute, secunde)
		print aer
		
		g = open('setfilename', "a")
		g.writelines(str(ae[0]) + "," + str(ae[1]) + "," + str(ae[2]) + "\n")
		i += 300
	g.close()